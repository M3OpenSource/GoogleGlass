...
public class AuthFilter implements Filter {
  private static final GridLogger LOG = GridLogger.getLogger(AuthFilter.class.getSimpleName());
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
    ...
    if (response instanceof HttpServletResponse && request instanceof HttpServletRequest) {
      ...
      if (httpRequest.getRequestURI().startsWith(httpRequest.getContextPath() + "/static") ||
          httpRequest.getRequestURI().equals(httpRequest.getContextPath() + "/oauth2callback") ||
          httpRequest.getRequestURI().equals(httpRequest.getContextPath() + "/notify")) {
      ...
      LOG.warn("Unexpected non HTTP servlet response. Proceeding anyway.");
      filterChain.doFilter(request, response);
    }
  }
